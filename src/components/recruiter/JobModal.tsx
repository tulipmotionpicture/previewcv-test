...
interface JobModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (jobId: number | null, data: Partial<Job>) => Promise<void>;
  job: Job | null;
...
export default function JobModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await onSave(job ? job.id : null, updateData);
      onClose();
    } catch (error) {
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
            <div className="space-y-2">
              ...
              <JobSearch
                title={jobTitleInput || formData.title}
                onChange={(val: JobTitle | null) => {
                  if (val) {
                    if (val.id > 0) {
                      setFormData((prev) => ({ ...prev, title: val.title }));
                      setJobTitleInput("");
                    } else {
                ...
                renderInput={({
                  ...
                  <input
                    type="text"
                    value={jobTitleInput || formData.title} // show input if typing, else form value
                    onChange={(e) => {
                      rOnChange(e);
                      setFormData((prev) => ({
                        ...prev,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <CountrySearch
                ...
                onChange={(c) => {
                  onChange({
                    target: { name: "country", value: c ? c.name : "" },
                  } as any);
                  onChange({
                    target: { name: "countryCode", value: c ? c.code : "" },
                  } as any);
            ...
            <div className="space-y-2">
              ...
              <StateSearch
                ...
                onChange={(s) => {
                  onChange({
                    target: { name: "state", value: s ? s.name : "" },
                  } as any);
                  onChange({
                    target: { name: "stateCode", value: s ? s.code : "" },
                  } as any);
                  onChange({ target: { name: "city", value: "" } } as any);
                }}
              />
            ...
            <div className="space-y-2">
              ...
              <CitySearch
                ...
                onChange={(c) => {
                  onChange({
                    target: { name: "city", value: c ? c.name : "" },
                  } as any);
                }}
              />
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 mb-2">
              <input
                  ...
                  if (e.key === "Enter" && !e.defaultPrevented) {
                    ...
                    if (
                      ...
                      if (!currentCats.includes(categoryInput.trim())) {
                        currentCats.push(categoryInput.trim());
                        onChange({
                          target: {
                            name: "categories",
                            value: currentCats.join(","),
                          },
                        } as any);
                      }
                      setCategoryInput("");
                ...
                className="flex-1 w-full px-4 py-2.5 bg-white dark:bg-[#282727] border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder-gray-400 dark:text-white"
              />
              <button
                type="button"
                onClick={() => {
                      ...
                      onClick={() => {
                        const currentCats = formData.categories
                          ? formData.categories
                              ...
                              .map((c) => c.trim())
                          : [];
                        const newCats = currentCats.filter(
                          (c) => c !== cat.trim(),
                        );
                        onChange({
                          target: {
                  ...
                  .map((cat, i) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const currentCats = formData.required_skills
                            ? formData.required_skills
                                .split(",")
                                .filter(Boolean)
                                .map((c) => c.trim())
                  ...
                  <SkillSearch
                    ...
                    onChange={(s) => {
                      if (s) {
                        if (s.id > 0) {
                          if (s.skillName) {
                            const currentSkills = formData.preferred_skills
                              ? formData.preferred_skills
                                  .split(",")
                                  .filter(Boolean)
                                  .map((x) => x.trim())
                          ...
                          if (e.key === "Enter" && !e.defaultPrevented) {
                            ...
                            if (prefSkillInput.trim()) {
                              const currentSkills = formData.preferred_skills
                                ...
                                : [];
                              if (
                                !currentSkills.includes(prefSkillInput.trim())
                              ) {
                    ...
                    if (prefSkillInput.trim()) {
                      const currentSkills = formData.preferred_skills
                        ? formData.preferred_skills
                            ...
                            .map((x) => x.trim())
                        : [];
                      if (!currentSkills.includes(prefSkillInput.trim())) {
                        currentSkills.push(prefSkillInput.trim());
                        onChange({
                          target: {
                            name: "preferred_skills",
                  ...
                  .map((cat, i) => (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const currentCats = formData.preferred_skills
                            ? formData.preferred_skills
                                .split(",")
                                .filter(Boolean)
                                .map((c) => c.trim())
