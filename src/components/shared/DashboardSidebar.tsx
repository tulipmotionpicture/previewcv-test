...
export default function DashboardSidebar<T extends string>({
    ...
    return (
        <aside
            className={`w-72 min-h-screen p-6 sticky top-0 h-screen flex flex-col transition-colors duration-200 ${isDark
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-lg transition-all duration-150 text-sm flex items-center ${showChevron ? "justify-between" : "gap-3"
                                } group cursor-pointer ${isActive
                                    ? "bg-teal-600 text-white"
                                    : "text-gray-600 dark:text-gray-400 hover:bg-[#0369A1]/5 dark:hover:bg-[#0369A1]/10 hover:text-[#0369A1] dark:hover:text-[#0EA5E9]"
                                }`}
                        >
                            <div className="flex items-center gap-3">
                                <span
                                    className={`transition-colors duration-150 ${isActive
                                        ? "text-white"
                                        : "text-gray-400 dark:text-gray-500 group-hover:text-[#0369A1] dark:group-hover:text-[#0EA5E9]"
                                        }`}
                                >
                                    {item.icon}
                                </span>
                            ...
                            {showChevron && (
                                <ChevronRight
                                    className={`w-4 h-4 transition-opacity duration-150 ${isActive ? "opacity-100" : "opacity-0 group-hover:opacity-50"
                                        }`}
                                />
                            )}
                        </button>
