 ...
 * logged-in candidate is not active/verified. Offers a "Resend verification
 * email" action with a 60s cooldown. Mirrors RecruiterVerificationBanner.
 */
export default function CandidateVerificationBanner() {
  ...
  if (loading || !candidateNeedsVerification(user)) return null;

  const handleResend = async () => {
    if (sending || cooldown > 0 || !user?.email) return;
    setSending(true);
    try {
    ...
    <div className="sticky top-0 z-50 w-full border-b border-amber-200 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/40">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-3 gap-y-1 px-4 py-2.5 text-center">
        ...
        <span className="text-sm text-amber-800 dark:text-amber-100">
          ...
          {user?.email ? ` (${user.email})` : ""} to apply for jobs.
        </span>
        <button
          type="button"
